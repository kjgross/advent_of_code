# Day 6 = Probably a Fire Hazard (grid toggle, switches)

import re, copy
directions = ['turn on 489,959 through 759,964',
...
big = []
for i in range(1000):
	big.append(copy.deepcopy(row_array))

for i in directions:
	if i.find("turn on") == 0:
		action = "turn on"
	elif i.find("turn off") == 0:
		action = "turn off"
	elif i.find("toggle") == 0:
		action = "toggle" 
	coords = re.findall(r'\d+', i)
	xa = int(coords[0])
	ya = int(coords[1])
	xb = int(coords[2])
	yb = int(coords[3])
